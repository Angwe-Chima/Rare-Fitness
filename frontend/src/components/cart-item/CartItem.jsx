...
const CartItem = () => {
  ...
  const {products, cartItems, addToCart, reduceCartItemQty, removeItemFromCart, getTotalCartAmount, getTotalCartItems, placedOrder} = useContext(ShopContext)
      ...
      {products.map(e => {
            if(cartItems[e.id] > 0){
              return <div className="cartItem" key={e.id}>
                      <img className="cartItem-image" src={e.imageUrl} alt={e.name} onClick={() => setSelectedProductId(e.id)}/>
                      <div className="cartItem__details">
                        <h3>{e.name}</h3>
                        <p>${formatCurrency(e.priceCents)}</p>
                      </div>
                      <div className="cartItem__quantity">
                        <CiCircleMinus onClick={() => reduceCartItemQty(e.id)} className="cartItem__quantity-change"/>
